...
with st.sidebar:
    ...
    with st.expander('Dashboard Approach', expanded=False):
        st.write('''
            ...
            ''', unsafe_allow_html=True)
    st.write('<a style="color:blue; text-decoration: none; font-size: 10px; font-weight: bold;" href="#">By @Alimghori</a>', unsafe_allow_html=True)
...
for seconds in range(500):
    ...
    # Creating KPI's for retail products
    ...
    with placeholder.container():
        ...
        with tab2:
            ...
            with fig_col1: # Store wise data
                ...
                locale.setlocale(locale.LC_ALL, '')
                total_category_df['Sales_labels'] = total_category_df['Sales_sqrt_new'].apply(lambda x: f"${locale.format_string('%d', x, grouping=True)}")
                st.markdown("### Streaming sales per square foot across the product categories for each store.")
                start_time = datetime.now()
                delta = timedelta(minutes=30)
                current_time = start_time + seconds * timedelta(seconds=50)
                current_time += timedelta(seconds=100)
